...
`interview.py` uses `Codex-haiku-4-5-20251001`. The system prompt is built fresh per request from the current questions in the DB — no caching. The model is forced (`tool_choice`) to answer every turn via the `interview_turn` tool; `call_claude()` returns `(display_text, completed, asking_question_id)` read straight from validated tool input. The route derives rating-button visibility from `asking_question_id` + the question's `question_type` in the DB (no text markers).
